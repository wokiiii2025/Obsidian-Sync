...
const translations: Record<Language, Record<TranslationKey, string>> = {
  en: {
    ...
    "settings.pluginSync.desc": "Sync only plugin IDs listed below. This sync plugin itself is always excluded and plugin files are counted separately.",
    "settings.pluginDirectories.name": "Plugin IDs to sync",
    "settings.pluginDirectories.desc": "One plugin ID per line, for example dataview or templater-obsidian. Leave empty to sync no plugin directories.",
  ...
  zh: {
    ...
    "settings.pluginSync.desc": "只同步下方列出的插件 ID。本同步插件自身始终排除，插件文件数会单独统计。",
    "settings.pluginDirectories.name": "要同步的插件 ID",
    "settings.pluginDirectories.desc": "每行一个插件 ID，例如 dataview 或 templater-obsidian。留空则不同步插件目录。",
